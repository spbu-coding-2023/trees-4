...
class RBTree<K : Comparable<K>, V> : BalancedTree<K, V, RBNode<K, V>>() {
    ...
    private fun balancedRemove(treeBranch: ArrayDeque<RBNode<K, V>?>) {
        ...
        while (parent != null) {
            ...
            brother ?: throw IllegalStateException("Balance of RB tree violated: different black height.")
            var nephewRight = brother.right
            var nephewLeft = brother.left

            //depending on nephew's colors, doing magic for balancing purposes
            if (nephewRight?.isRed != true && nephewLeft?.isRed != true && parent.isRed) {
                brother.isRed = true
                parent.isRed = false
                break
            } else if (nephewRight?.isRed != true && nephewLeft?.isRed != true && !parent.isRed) {
                brother.isRed = true   //parent is black --> go higher up
            ...
            if (son == parent.right) {
                if (nephewRight?.isRed == true && nephewLeft?.isRed != true) {
                ...
                if (nephewLeft?.isRed == true && nephewRight?.isRed != true) {
